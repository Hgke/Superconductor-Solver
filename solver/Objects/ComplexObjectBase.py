...
from Objects.MutualInductance import MutualInductance
...
class ComplexObjectBase(ElementBase):
    ...
    def add_MutualInductance(self, name, L1, L2,k, loc):
        """
        Adds a mutual inductance object
        :param loc: nodes in a circuit to what the element is connected (zero is a ground)
        :param L1: a first inductance
        :param L2: a second inductance
        :param k: a mutual inductance parameter, M = k * sqrt(L1 * L2)
        """
        new_obj = MutualInductance(loc=loc, L1=L1, L2=L2, k=k)
        new_obj.name = f'{self.name}_{name}'
        self._new_names_obj.append(new_obj)
