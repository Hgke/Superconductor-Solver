...
class DFF(ComplexObjectBase):
    ...
    def create_elements(self, sk):
        ...
        #self.add_L(name='L3', val=0.6, loc=[sk[1], sk[6]]) - это было, с этим схема для управления не работает (нет норм работы Splitter)
        self.add_L(name='L1', val=3.21, loc=[sk[4], sk[5]])
        self.add_L(name='L2', val=1.2, loc=[sk[5], sk[2]])
        self.add_L(name='L3', val=1, loc=[sk[1], sk[6]])

        self.add_ib(name='Ib1', val=1.3, loc=[sk[4]])
